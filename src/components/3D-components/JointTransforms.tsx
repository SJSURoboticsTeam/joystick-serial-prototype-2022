...
export const JointTransforms = ({ links, base, setTransforms }: { links: Solve3D.Link[]; base: Solve3D.JointTransform; setTransforms: (state: Solve3D.JointTransform[]) => void }) => {
  ...
  const jointTransforms = useMemo(
    () =>
      Array.from({ length: links.length + 1 }).map((_, index) => {
        return (
          <mesh key={index}>
            <boxBufferGeometry args={[0.1, 0.1, 0.5]} />
